...
detect_available_cpu_count() {
  ...
  local sysctl_bin="${RUNNER_SYSCTL_BIN:-sysctl}"
  ...
  if [ -z "${cpu_count}" ] && command -v "${sysctl_bin}" >/dev/null 2>&1; then
    cpu_count="$("${sysctl_bin}" -n hw.ncpu 2>/dev/null || true)"
    cpu_count="${cpu_count//[[:space:]]/}"
  fi
...
Notes:
  ...
  - Runner CPU limits are whole-number percentages; 100% equals one CPU,
