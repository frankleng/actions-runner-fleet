...
restore_output="$(
  ...
  MY_ENTERPRISE_RUNNER_REGISTRATION_TOKEN="enterprise-smoke-token" \
...
grep -Fq $'enterprise-smoke-token\t--url https://github.com/enterprises/example-enterprise --replace-existing --no-start enterprise-runner-1' "${operations_log}"
...
grep -Fq "enterprise target https://github.com/enterprises/example-enterprise" <<< "${restore_output}"
